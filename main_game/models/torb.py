...
class Torb(models.Model):
    class Action(models.TextChoices):
        ...
        SOLDIERING = "soldiering", "Soldiering"
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=models.Q(hp__gte=0, max_hp__gte=0) & models.Q(hp__lte=models.F("max_hp")),
                name="torb_hp_in_range",
            ),
            models.UniqueConstraint(
                fields=["colony", "private_ID"],
                name="unique_torb_private_id_per_colony",
            ),
        ]
    ...
    def adjust_hp(self, adjust_amount, context="an unknown source"):
        ...
        logger.debug(
            f"Colony {self.colony.id} '{self.colony.name}' Torb {self.private_ID} '{self.name}' adjusted hp amt {adjust_amount} context: {context}"
        )
        ...
        logger.debug(
            f"Colony {self.colony.id} '{self.colony.name}' Torb {self.private_ID} '{self.name}' died, context: {context}"
        )
        self.save()
        self.set_action(action="dead")
        membership = self.army_torb.first()
        if membership:
            membership.remove_from_army()
    ...
    def set_action(self, action: str, context_torb=None):
        ...
        logger.debug(
            f"Colony {self.colony.id} '{self.colony.name}' Torb {self.private_ID} setting action {action} context torb: {context_torb}"
        )
        # If prior action was breeding, ensure paired torb is also no longer breeding
        if self.action == "breeding" and self.context_torb:
            logger.debug(
                f"Colony {self.colony.id} '{self.colony.name}' Torb {self.private_ID} Already breeding with {self.context_torb}"
            )
