...
class Army(models.Model):
    ...
    scout_target = models.ForeignKey(
        "colony", blank=True, null=True, on_delete=models.SET_NULL, related_name="scouting_armies"
    )
    attack_target = models.ForeignKey(
        "colony", blank=True, null=True, on_delete=models.SET_NULL, related_name="attacking_armies"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=models.Q(morale__gte=0, morale__lte=100),
                name="army_morale_in_range",
            )
        ]
    ...
    def new_round(self, rng=None):
        rng = rng or random.Random()
        self.purge_soldiers()
        self.scout_colony(rng=rng)
        self.attack_colony(rng=rng)
        self.train_soldiers(rng=rng)
        self.scout_target = None
        self.attack_target = None
        self.save()

    def train_soldiers(self, rng=None):
        rng = rng or random.Random()
        for torb in self.colony.torbs.all():
            if torb.action == "training":
                ...
                torb.save()
                ArmyTorb.add_to_army(self, torb, rng=rng)
    ...
    def set_scout_target(self, scout_target_id):
        ...
        if not scout_target_id:
            ...
            StoryText.objects.create(
                ...
                story_text="Our scouting plans have been canceled.",
                timestamp=Now(),
            )
            return
        ...
        if scout_target_colony == self.colony:
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="scout",
                story_text="We likely won't get any new information trying to scout ourselves.",
                timestamp=Now(),
            )
            return

        if self.scout_target and scout_target_colony in self.colony.discovered_colonies.all():
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="scout",
        ...
        elif scout_target_colony in self.colony.discovered_colonies.all():
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="scout",
            ...
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="scout",
                story_text="Our scouts will go out and learn about the as-yet-unknown colony.",
                timestamp=Now(),
            )
        self.scout_target = scout_target_colony
        self.save()

    def scout_colony(self, rng=None):
        rng = rng or random.Random()
        ...
        random_enemy_torb_power = rng.choice(
            [soldier.power for soldier in colony_to_scout.army.army_torbs.all()]
        )
        random_ally_soldier = rng.choice([soldier for soldier in self.army_torbs.all()])
        ...
        if rng.uniform(0, 1) > (random_enemy_torb_power / random_ally_torb_resilience):
        ...
        self._scout_failed(
            random_ally_soldier,
            colony_to_scout,
            random_enemy_torb_power,
            random_ally_torb_resilience,
            rng=rng,
        )
        self.scout_target = None
        self.save()
    ...
    def set_attack_target(self, attack_target_id):
        ...
        if not attack_target_id:
            ...
            StoryText.objects.create(
                ...
                story_text="Our attack plans have been canceled.",
                timestamp=Now(),
            )
            return
        ...
        if attack_target_colony == self.colony:
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="combat",
                story_text="Our soldiers will never agree to attack their own colony!",
                timestamp=Now(),
            )
            return
        if attack_target_colony not in self.colony.discovered_colonies.all():
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="combat",
                story_text="We can't send our soldiers out without knowing where they're going!",
                timestamp=Now(),
            )
            return
    ...
    def attack_colony(self, rng=None):
        rng = rng or random.Random()
        ...
        if self.colony.num_soldiers == 0:
            StoryText.objects.create(
                colony=self.colony,
                story_text_type="combat",
                story_text="You ordered your Torbs to attack, but without training they just bumbled about.",
                timestamp=Now(),
            )
            return False
        ...
        if colony_to_attack.num_soldiers == 0:
            ...
            won_fight = self.battle_army(enemy_army=colony_to_attack.army, rng=rng)
        if won_fight:
            self._attack_successful(colony_to_attack, rng=rng)
        else:
            self._attack_failed(colony_to_attack)
        self.attack_target = None
        self.save()

    def battle_army(self, enemy_army, rng=None, max_rounds=1000) -> bool:
        rng = rng or random.Random()
        if self.morale <= rng.randrange(1, 50):
            return False
        for _ in range(max_rounds):
            allies = list(self.army_torbs.select_related("torb").filter(torb__is_alive=True))
            enemies = list(enemy_army.army_torbs.select_related("torb").filter(torb__is_alive=True))
            if not enemies:
                return True
            if not allies:
                return False
            before = (sum(item.torb.hp for item in allies), sum(item.torb.hp for item in enemies))
            self.torb_fight(rng.choice(allies), rng.choice(enemies), rng=rng)
            allies_after = (
                self.army_torbs.filter(torb__is_alive=True).aggregate(models.Sum("torb__hp"))[
                    "torb__hp__sum"
                ]
                or 0
            )
            enemies_after = (
                enemy_army.army_torbs.filter(torb__is_alive=True).aggregate(models.Sum("torb__hp"))[
                    "torb__hp__sum"
                ]
                or 0
            )
            if before == (allies_after, enemies_after):
                return False
        return False

    def torb_fight(self, ally_army_torb, enemy_army_torb, rng=None):
        rng = rng or random.Random()
        logger.debug(
            f"{self.colony.name}: Torb {ally_army_torb} is fighting {enemy_army_torb.torb.colony}'s {enemy_army_torb}"
        )
        ally_army_torb_attack = rng.uniform(1, ally_army_torb.power)
        ally_army_torb_defense = rng.uniform(1, ally_army_torb.resilience)

        enemy_army_torb_attack = rng.uniform(1, enemy_army_torb.power)
        enemy_army_torb_defense = rng.uniform(1, enemy_army_torb.resilience)

        ally_army_torb_speed = rng.uniform(1, ally_army_torb.active_alleles["agility"])
        enemy_army_torb_speed = rng.uniform(1, enemy_army_torb.active_alleles["agility"])
        logger.debug(
            f"{self.colony.name}: Torb {ally_army_torb} has speed: {ally_army_torb_speed}, attack: {ally_army_torb_attack} and defense: {ally_army_torb_defense}"
        )
        logger.debug(
            f"{enemy_army_torb.torb.colony}: Torb {enemy_army_torb} has speed: {enemy_army_torb_speed}, attack: {enemy_army_torb_attack} and defense: {enemy_army_torb_defense}"
        )
        ...
        if ally_army_torb_speed > enemy_army_torb_speed:
            ...
            enemy_army_torb.torb.adjust_hp(
                enemy_hp_adjust, context=f"defending against {self.colony.name}'s Army"
            )
            if enemy_army_torb.torb.is_alive:
                ally_hp_adjust = round(min(0, ally_army_torb_defense - enemy_army_torb_attack), 0)
                ally_army_torb.torb.adjust_hp(
                    ally_hp_adjust,
                    context=f"in a battle against {enemy_army_torb.torb.colony.name}",
                )
        else:  # Violates DRY
            ally_hp_adjust = round(min(0, ally_army_torb_defense - enemy_army_torb_attack), 0)
            ally_army_torb.torb.adjust_hp(
                ally_hp_adjust, context=f"in a battle against {enemy_army_torb.torb.colony.name}"
            )
            if enemy_army_torb.torb.is_alive:
                enemy_hp_adjust = round(min(0, enemy_army_torb_defense - ally_army_torb_attack), 0)
                enemy_army_torb.torb.adjust_hp(
    ...
    def _attack_successful(self, enemy_colony, rng=None):
        rng = rng or random.Random()
        stolen_food_amount = 0
        if enemy_colony.food > 0:
            minimum = min(enemy_colony.food, round(self.army_power))
            stolen_food_amount = (
                enemy_colony.food
                if minimum >= enemy_colony.food
                else rng.randint(minimum, enemy_colony.food)
            )
            self.colony.adjust_food(stolen_food_amount)
            enemy_colony.adjust_food(-stolen_food_amount)
        StoryText.objects.create(
            colony=self.colony,
            story_text_type="combat",
    ...
    def _scout_lucky_escape(self, colony_to_scout):
        ...
        StoryText.objects.create(
            colony=colony_to_scout,
            story_text_type="enemy",
            story_text="An enemy soldier was seen scouting your colony and was too quick to be identified.",
            timestamp=Now(),
        )
    ...
    def _scout_failed(
        ...
        random_ally_soldier.torb.adjust_hp(
            -1 * damage_to_take, context="an enemy soldier while scouting"
        )
...
class ArmyTorb(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["torb"], name="unique_army_membership_per_torb")
        ]
    ...
    def resilience(self):
        return round(
            (self.active_alleles["vitality"] * self.active_alleles["sturdiness"]) ** 0.5, 2
        )

    @classmethod
    def add_to_army(cls, army, torb, rng=None):
        rng = rng or random.Random()
        # Randomly select alleles for power and resilience when adding the Torb to the army
        strength_allele = rng.choice(torb.genes["strength"])
        agility_allele = rng.choice(torb.genes["agility"])
